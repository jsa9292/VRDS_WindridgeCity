...
Shader "EasyRoads3D/ER Blend UV4" {
    Properties {
   ...
   //		#pragma surface surf Standard  fullforwardshadows Lambert decal:blend
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float4 _main1 = tex2D (_SecondTex, IN.uv4_SecondTex) * _Color2;
			
		//	float4 _mask = tex2D (_maskTex, IN.uv_MainTex);
			
            fixed4 nrm = tex2D(_BumpMap, IN.uv_MainTex);
            fixed4 nrm1 = tex2D(_BumpMap2, IN.uv4_SecondTex);
		...
		//	if(_mask.a < c.a)c.a = _mask.a;
            ...
            o.Normal = UnpackNormal(lerp(nrm1, nrm, c.a));
        //    o.Albedo = lerp(_main.rgb, _main1.rgb, c.a);
            o.Albedo = lerp(_main1.rgb, _main.rgb, c.a);
            o.Alpha = alpha;
