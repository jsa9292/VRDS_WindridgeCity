...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarController : MonoBehaviour
    {
        ...
        [SerializeField] private static int NoOfGears = 10;
        ...
        [SerializeField] public float Revs { get; private set; }
        ...
        private void CalculateRevs()
        {
            ...
            CalculateGearFactor();
            var gearNumFactor = (m_GearNum+1)/(float) (NoOfGears+1);
            var revsRangeMin = ULerp(0f, m_RevRangeBoundary, CurveFactor(gearNumFactor));
            var revsRangeMax = ULerp(m_RevRangeBoundary, 1f, gearNumFactor);
            Revs = ULerp(revsRangeMin, revsRangeMax, m_GearFactor);
        }
		public void Move(float steering, float accel, float footbrake, float handbrake, float reverse)
		{
			for (int i = 0; i < 4; i++)
			{
				Quaternion quat;
			...
			AccelInput = accel = Mathf.Clamp (accel, -1, 1)*reverse;
        ...
        private void CheckForWheelSpin()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                WheelHit wheelHit;
                m_WheelColliders[i].GetGroundHit(out wheelHit);
				//if (i <2) Debug.Log (wheelHit.forwardSlip);
                // is the tire slipping above the given threshhold
				if (CurrentSpeed < 1f || CurrentSpeed == 0) return;
